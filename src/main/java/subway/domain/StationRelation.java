...
public class StationRelation {
    ...
    private final WeightedMultigraph<String, DefaultWeightedEdge> graphDistance = new WeightedMultigraph(
        DefaultWeightedEdge.class);
    private final WeightedMultigraph<String, DefaultWeightedEdge> time = new WeightedMultigraph(
        DefaultWeightedEdge.class);
    ...
    private void madeBundangInfo() {
        ...
        graphDistance.setEdgeWeight(graphDistance.addEdge(Station.YANGZAE, Station.YANGZAE_FOREST),
            10);
        time.setEdgeWeight(time.addEdge(Station.YANGZAE, Station.YANGZAE_FOREST), 3);
    }

    private void madeThirdInfo() {
        graphDistance.setEdgeWeight(
            graphDistance.addEdge(Station.EDUCATION_COLLEGE, Station.SOUTH_TERMINAL), 3);
        time.setEdgeWeight(time.addEdge(Station.EDUCATION_COLLEGE, Station.SOUTH_TERMINAL), 2);

        graphDistance.setEdgeWeight(graphDistance.addEdge(Station.SOUTH_TERMINAL, Station.YANGZAE),
            6);
    ...
    private void madeSecondInfo() {
        graphDistance.setEdgeWeight(
            graphDistance.addEdge(Station.EDUCATION_COLLEGE, Station.GANGNAM), 2);
    ...
    public TravelResult shortestDistance(Station start, Station dest) {
        try {
            ...
            throw new IllegalArgumentException(ErrorMessage.NO_CONNECT_STATION.getMessage());
        }
    }
    ...
    private int calculateRouteByDistance(List<String> route) {
        int sum = 0;
        for (int i = 0; i < route.size() - 1; i++) {
            sum += (int) graphDistance.getEdgeWeight(
                graphDistance.getEdge(route.get(i), route.get(i + 1)));
        }
        return sum;
    }
